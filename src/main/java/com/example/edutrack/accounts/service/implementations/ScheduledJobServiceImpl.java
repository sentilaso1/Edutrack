...
import com.example.edutrack.accounts.repository.ScheduledJobLogRepository;
...
public class ScheduledJobServiceImpl implements ScheduledJobService{
        ...
        @Autowired private ScheduledJobLogRepository logRepo;
        ...
        public void runJobNow(Long id) {
                ...
                ScheduledJob job = jobRepo.findById(id).orElseThrow();
                // call scheduler.execute(job)
        }

        @Override
        public List<ScheduledJobLog> getJobLogs(Long jobId) {
                return logRepo.findByJobIdOrderByExecutedAtDesc(jobId);
        }
